...
while true ; do
  VERSION_EXISTS="$(
    curl -s -o /dev/null -w "%{http_code}" "https://github.com/AthenZ/terraform-provider-athenz/releases/download/$PRERELEASE_VERSION_WITH_PREFIX/terraform-provider-athenz_${PRERELEASE_VERSION}_linux_amd64.zip"
  )"
